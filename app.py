...
def model_predict(cleaned_text):  
  for word in cleaned_text.split(' '):
    if word in ['kill','murder','fuck','bitch','war']:
      return 1#'Offensive'
  return 0#'Non Offensive'  

def model_predict_proba(cleaned_text):  
  for word in cleaned_text.split(' '):
    if word in ['kill','murder','fuck','bitch','war']:
...
def cleaner(orig_tweet):
    ...
    #replacing shortcut words with their abbrevations
    temp=[]
    for word in orig_tweet.split():
        if word in abbreviations.keys():
            temp.append(abbreviations[word])
        else:
            temp.append(word)

    orig_tweet=(" ".join(temp)).strip()
    ...
    # lemmatization
    orig_tweet = [lemma.lemmatize(word=w) for w in orig_tweet]

    # This converts the list of words into space-separated strings
    orig_tweet = ' '.join(orig_tweet)
...
def predict():
	...
	if request.method == 'POST':
		...
		my_prediction = model_predict(cleaned_text)
